...
from api.watchlist import add_to_watchlist, get_watchlist_count

# Company info / financials / price plotting
def user_page(session, supabase):
    ...
    fig.add_trace(go.Candlestick(x=df_plot['date'], open=df_plot['open'], high=df_plot['high'], low=df_plot['low'], close=df_plot['close'], name='Price', increasing_line_color='rgba(0,200,0,1)', decreasing_line_color='rgba(200,0,0,1)', showlegend=False), row=1, col=1)
    if 'bb_upper_20' in df_plot.columns and 'bb_lower_20' in df_plot.columns and 'bb_sma_20' in df_plot.columns:
    ...
    if 'buy_signal' in df_plot.columns and 'sell_signal' in df_plot.columns:
        ...
        if not buys.empty:
            fig.add_trace(go.Scatter(x=buys['date'], y=buys['close'], mode='markers', marker=buy_marker, name='Buy Signal', hovertemplate='Buy<br>%{x|%Y-%m-%d}<br>Price: %{y:.2f}<extra></extra>'), row=1, col=1)
        if not sells.empty:
            fig.add_trace(go.Scatter(x=sells['date'], y=sells['close'], mode='markers', marker=sell_marker, name='Sell Signal', hovertemplate='Sell<br>%{x|%Y-%m-%d}<br>Price: %{y:.2f}<extra></extra>'), row=1, col=1)

    if 'macd' in df_plot.columns and 'macd_signal' in df_plot.columns:
        fig.add_trace(go.Bar(x=df_plot['date'], y=df_plot['macd_hist'] if 'macd_hist' in df_plot.columns else [0]*len(df_plot), name='MACD Hist', marker=dict(), showlegend=False), row=2, col=1)
    ...
    fig.update_layout(dragmode='pan', height=720, margin=dict(l=20, r=20, t=40, b=20), xaxis=dict(rangeslider=dict(visible=False)))
        ...
        if not selected_tfs:
            ...
            for case in ("min", "average", "greedy"):
                ...
                for tf in selected_tfs:
                    ...
                    if dates and values:
                        any_series = True
                        fig_eq.add_trace(go.Scatter(x=[pd.to_datetime(d) for d in dates], y=values, mode="lines+markers", name=tf, hovertemplate="%{x|%Y-%m-%d}<br>Equity: %{y:.4f}<extra></extra>"))
                if any_series:
                    fig_eq.update_layout(title=f"{case.title()} case — Cumulative equity", height=320, margin=dict(l=20, r=20, t=30, b=10))
                    st.plotly_chart(fig_eq, use_container_width=True)
                else:
                    st.info(f"No equity data for {case} case to plot.")

    with st.expander("Other criteria & ideas"):
        st.write("- Sharpe / Sortino ratio")
        st.write("- Transaction cost sensitivity (simulate bps)")
        st.write("- Walk-forward / Monte Carlo tests")
        st.write("- Compare vs buy-and-hold benchmark")

    st.markdown("---")
    watch_col1, watch_col2 = st.columns([3, 1])
    with watch_col1:
        st.write(f"Add **{ticker}** to your watchlist")
    with watch_col2:
        if st.button("+ Add to watchlist"):
            try:
                user_id = getattr(session.user, 'id', None)
                if not user_id:
                    st.error("You must be logged in to add to the watchlist.")
                else:
                    out = add_to_watchlist(user_id=user_id, ticker=ticker)
                    if out.get("status") == "added":
                        st.success(f"{ticker} added to your watchlist")
                    elif out.get("status") == "exists":
                        st.info(f"{ticker} already in your watchlist")
                    else:
